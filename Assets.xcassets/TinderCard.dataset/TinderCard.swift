...
class TinderCard: UIView,UIScrollViewDelegate {
    ...
    /*        self.addSubview(self.defaulCardImageView)
      ...
      //  self.addSubview(self.shareFabButton)
       ...
       /* DispatchQueue.main.asyncAfter(deadline: .now() ) { // Despues de hacer tap, va al menu Home
        ...
        var initWeb = CGFloat.abs(0)
        var rectWeb = CGRect.init(x: initWeb, y: frame.size.height, width: frame.size.width, height: heightWebView)
